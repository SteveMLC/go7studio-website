...
The first version of our CE platform asked the LLM to write a quiz the moment a student opened a lesson. It looked beautiful in a demo. We almost shipped it.

Then I sat down and asked the regulator's question: if a student fails a quiz, retries it, and passes the second time, what did they actually see? A different prompt's response. A different shape. Possibly a different difficulty. No record. No audit trail. No way to claim that two students of the same skill level took comparable assessments.

That's not a quiz engine. That's a vibe.

This post is about what we built instead — a Bloom-tagged question bank with distribution-aware selection — and why it's the right shape for any compliance-grade learning platform we ship through Go7Studio. The example is the [NEALAC Learning Hub](https://drannnealac.com), built with Dr. Ann Leonard-Zabel as the public-facing CE provider. The architecture decisions generalize to every edtech build we've shipped since.

<HeroCallout eyebrow="Assessment architecture" title="Runtime quiz generation looked impressive until it had to answer an audit." body="A compliance-grade quiz needs reproducible selection, source anchors, and a record of what each student saw. A Bloom-tagged bank gives the platform those properties before any learner opens a lesson." />

<DecisionMatrix title="Quiz architecture trade-offs" columns='["Architecture","What it optimizes for","Where it fails"]' rows='[{"label":"LLM on demand","values":["Fresh-looking quizzes with no authoring backlog.","Non-deterministic difficulty, slow lesson loads, and no durable record of the exact instrument shown."]},{"label":"Static bank","values":["Fast rendering and simple audit records.","Random selection can still collapse into recall-only quizzes if the bank has no cognitive distribution rules."]},{"label":"Bloom-tagged bank","values":["Deterministic sampling across recall, application, and higher-order categories.","Requires generation-time tagging, coverage checks, and rejection paths before launch."]}]' />
...
The framing that finally clicked: a quiz is not content the LLM writes on demand. A quiz is a *measuring instrument*, and an instrument needs calibration, distribution control, and a reproducible read.
...
Notice the rationale on each answer, including the wrong ones. That's what lets us run distractor-quality audits without re-engaging a model. A good distractor is plausibly wrong; a bad one is obviously wrong. We score every distractor at write time so the bank itself can be filtered.

## Section 3: Distribution-aware selection

Here's where most quiz engines that have a bank still fail: they pick three random rows.
...
Why this composition? A passing CE quiz that tests only recall doesn't demonstrate competency — it demonstrates lookup. Distribution control is how we say "the student understands the material at three cognitive levels," not "the student memorized terminology."
...
A platform the regulator can audit. A platform where two students of the same skill level take comparable assessments. A platform that doesn't go down when the model API does. A platform whose quiz quality is owned by the team that built it, not by whatever the model decides to write at the moment of impression.

All 10 lines of our NEALAC verification suite passed on the first run, partly because we built the bank model from day one. Distribution checks, no-overlap checks, anchor checks, eligible-flag checks — all of them were *expected to pass* because the architecture made them tractable.

If you want to see what this looks like in a live, regulator-facing CE platform, [drannnealac.com](https://drannnealac.com) is hosting the result. The platform is small enough to read in an afternoon and rigorous enough to survive an audit. That's the shape we optimize for.
