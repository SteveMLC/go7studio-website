...
If you're running a fixed-budget AI studio, you can't afford to route everything to the strongest model. I learned that the expensive way.

The interesting question isn't which model is best. It's which model goes where. Best-model-per-call beats best-model-overall every time, because the cost difference between routing well and routing badly compounds across thousands of calls per day.
...
Five families wired through one harness because none of them is best at everything. Opus catches errors GPT-5.4 misses. GPT-5.4 produces content faster than Opus. Kimi handles long contexts cheaper than either. MiniMax runs heartbeat-rate inference for less than the price of a coffee per month. Sonnet is the workhorse middle that fills the gaps.

Route every call to Opus and I'm out of budget in a week. Route every call to MiniMax and my audit calls miss things that matter. The routing layer is the answer to "how do you afford to ship this much content."
...
When somebody on the team asks "which model should I use for X," the answer isn't a model name. It's "what call type is this?" Once you know the call type, the model is determined. The discipline is classifying the call correctly.
...
Hydra — my multi-strategy trading desk — runs on MiniMax. Numbers: 4.5s per call, $30/month, 15K calls per window, currently at about 1% utilization (31 calls/hr against a 3,000 calls/hr ceiling).
...
It's the wrong model for one-shot reasoning. If you need a single answer that has to be right, don't call MiniMax. The shallow-but-cheap profile is the whole point; using it for review or audit is using a hammer to drive screws.

The pattern that works: route any high-frequency low-stakes inference to MiniMax. Heartbeat probes for agent liveness. Novelty hashes on agent output. Light classification of incoming events. "Should this trigger a deeper review?" gates. None of those needs Opus. All of them love MiniMax's cost structure.
...
A call type with 95% accept rate has cost-per-accepted-output close to its cost-per-call. A call type at 80% accept rate runs 25% higher than naive cost-per-call would suggest. The 5% of calls you re-run are real money.

Accept rate is part of the cost model. A 20%-cheaper model at 70% accept rate is worse than a 20%-more-expensive model at 95% accept rate. Cost-per-call is a vanity metric. Cost-per-accepted-output is the metric that survives a budget meeting.
...
If you're running a fixed-budget AI shop and you find yourself routing everything to the strongest model, the leverage isn't in finding a stronger model. It's in routing better. The strongest model isn't the one your budget rewards. The strongest routing is.
...
<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  ...
  <p className="mt-3 text-white/70">One post a month from a production system that's currently breaking and getting fixed. The series so far: the agent harness, the bypass casualty, the operator role, the config bottleneck, the work-order field manual, and now the routing matrix. No fluff.</p>
