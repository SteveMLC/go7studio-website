...
The first version of our CE quiz generator was one prompt: "read this lesson and write three quiz questions." It produced fluent, well-formatted, completely uniform output. Every question on every lesson was a "what is the definition of X" recall.
...
This post is the architecture we shipped instead — three passes, three prompts, one tagger, one bank — and the per-course economics that make it cheap enough to use on every CE course we build.

<HeroCallout eyebrow="Assessment pipeline" title="One prompt produced one cognitive shape. Three passes produced a course." body="The generator works because each pass has a narrow job, then a separate tagger rejects drafts whose Bloom level does not match the pass that produced them." />

<DecisionMatrix title="Why the generator runs in passes" columns='["Pass","What it writes","What it prevents"]' rows='[{"label":"Factual","values":["Recall and understanding questions tied to transcript timestamps.","A quiz with no source anchor or audit path."]},{"label":"Application","values":["Scenario stems that require using the concept, not repeating the wording.","The common collapse into three versions of the same definition question."]},{"label":"Slide-specific","values":["Questions anchored to diagrams, tables, and slide text when the transcript only gestures at them.","Losing visual material because the speaker did not read every slide aloud."]},{"label":"Bloom tagger","values":["A second read that classifies the rendered draft before it enters the bank.","Trusting the generation prompt alone when the output does not match its requested shape."]}]' />

## Section 1: The single-pass trap

The default failure mode of LLM quiz generation is shape collapse. You ask for "three questions" without specifying their cognitive shape, and the model — trained on internet text — defaults to the most common shape on the internet: definitional recall.
...
The transcript anchor is load-bearing. It lets us audit the question against the lecture and link to the exact timestamp from the explanation page after a student answers — so a student who got it wrong can land at the precise moment the answer was given.

## Section 4: Pass 2 — Application

Pass 2 is the prompt that produces the questions students actually find hard — and the one that distinguishes a real assessment from a vocabulary test.
...
The "scenario must NOT appear verbatim in the transcript" rule is what forces real application. The model can't lift its scenario from the lecture — it has to construct one.
...
The reason cost matters: it lets the same architecture run on every course we build. There's no "premium tier where we run the proper generator." Every course gets the 3-pass treatment because the marginal cost is sub-dollar. The expensive thing is shaping the prompts the first time. After that, the pipeline runs.

If you're building a CE platform, an L&D platform, or any assessment engine with a regulator looking over your shoulder, this is the architecture we'd point you at. If you'd rather have us build it, we will.
