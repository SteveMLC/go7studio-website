...
tags:
  ...
  - canary-tests
status: "draft"
seoTitle: "Nineteen Bugs, Five Failure Patterns: Pipeline Atlas | Go7Studio"
...
This is the atlas of every bug we've shipped a fix for in the last three months across our content-ingestion pipeline. Nineteen of them, drawn from real postmortems with real commits. They are not interesting one at a time. They are interesting because they cluster into five recurring families, and once you can name the family, you can spot the next instance before it ships.

A postmortem without a canary is just a story. Each of these bugs has a canary test that runs before any pipeline-touching merge — the test is the rule that says "this specific failure mode cannot recur silently." Right now there are 39 of them and they all pass.
...
Different surfaces. Five underlying patterns. Walk through each.
...
Every dashed-line "bypass" arrow is a guard you do not have. The shortcut is the bug.

The fix in every case was the same shape: move the guard *out of* the optional step and *into* a step that always runs, no matter the route. Cleanup is optional. Contamination check is not. Consolidation before modernize is not optional either, regardless of how badly the operator wants to unblock the queue.
...
Discovery moment: HIT 101 published with 206 images, most of them logos and decorative. HIT 105 published with 271 images, same problem. Pipeline never filtered. The standalone scripts had been doing the filtering when run manually, and we'd just stopped running them manually.
...
Bug 8 is the most expensive bug in this atlas. It is also the most systemic.
...
Twelve courses bypass-routed (via Bug 7) plus an unknown count from earlier waves had this happen. The output was *coherent*. Native English. Pedagogically reasonable. Completely fabricated.
...
`extract_imscc` would then try to parse the shadow file as an IMSCC archive and fail on what was actually encoded HFS+ resource fork data. We were getting silent extraction failures with no indication that the source file even existed correctly elsewhere.
...
The rule isn't "fix the bug." The rule is "make the bug impossible to recur." Those are different things. A fix without a canary depends on the next operator remembering. A fix *with* a canary depends on the next operator running `run_canaries.py`, which they already do because it gates merges.
...
<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  ...
  <p className="mt-3 text-white/70">If your content pipeline is leaking money — fabricating where it should halt, bypassing where it should guard, or running standalone scripts that should be steps — that's what we do at Go7Studio. Every fix gets a canary.</p>
...
The discipline of writing the atlas down is what makes the next bug findable. Postmortems by themselves are stories. Postmortems organized by pattern family are a debugging vocabulary.
