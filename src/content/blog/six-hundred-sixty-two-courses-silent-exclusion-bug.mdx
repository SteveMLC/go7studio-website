...
It was lying to my face.

A `.imscc` recursion bug was hiding rich source files inside the queue. Items looked in-flight, with statuses transitioning and locks rotating, but the extractor couldn't reach the actual content. Every "in queue" item was potentially unprocessable, and there was no field on the dashboard that could tell me which ones.

This is the postmortem on a class of bug I've started to call *silent exclusion* — when your queue contains items that are technically there, technically tracked, technically progressing through statuses, but architecturally unreachable.
...
The thing the dashboard didn't show, because there was no field for it: how many of those items were *extractable*. Not "have we tried to extract" — that was a column. Not "is there a source file" — that was also a column. *Extractable*. Has there been a successful extraction with surviving content above threshold?

That wasn't a column, so it wasn't a filter, and it wasn't a histogram. The dashboard reported queue health. It did not report queue *processability*.
...
The outer manifest produced a clean output that *looked* like extraction had succeeded. Title extracted. Section structure extracted. Module titles extracted. The actual lesson content lived one level deeper, inside each `module_N.imscc`, and the extractor never got there.

Downstream, the pipeline saw a course with titles, structure, and zero lesson content. With Bug 8 (`intent_extract surviving_words=0` continued — see [the 19-bug atlas](/blog/nineteen-bugs-failure-pattern-atlas)), the modernize step then generated coherent generic content from the titles. With Bug 8 fixed, those courses now halt at extract. Before that patch landed, they shipped fabricated.
...
The deeper failure was in the status vocabulary.

The state machine has 12 statuses: `queued`, `locked`, `extracting`, `extracted`, `preflighting`, `preflighted`, `composing`, `composed`, `modernizing`, `modernized`, `enriching`, `seeding`, `seeded`, `halted`, `triage`. Each describes intent or progress. None describes *processability*.
...
Otherwise your dashboards are full of beautifully colored cells that don't mean what their column headers say. That's not metrics, that's decoration.
...
<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  ...
  <p className="mt-3 text-white/70">If your pipeline dashboards report "all green" and you can't tell which items are actually processable, your status vocabulary is the bug. That's the kind of work Go7Studio fixes. Small studio, real receipts.</p>
