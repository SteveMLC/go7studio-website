...
Yesterday I caught myself debugging a sanitize bug for the second time. I wrote the fix eighteen days ago and forgot.

The fix was already in a memory file with a one-line summary in my MEMORY.md index. The thirty seconds I would have spent reading that index would have saved the two hours I spent re-deriving the same answer. This post is about that thirty seconds.
...
At the top of my memory directory there's one file, `MEMORY.md`. Every topical memory file I write gets one line in it: the link, a hyphen, and a one-sentence summary of what's inside. No more, no less.
...
That's a real excerpt. MEMORY.md loads automatically on every session boot, so the full backlog of "what I've learned, with pointers to the receipts" is the first thing on screen.
...
Predictable shape, predictable scan path. After thirty topical files I can find any past decision in under a minute. The audit-log compound isn't the writing — it's the re-reading.
...
On 2026-04-23, an audit caught 212 of 662 published courses containing raw TAACCCT/DOL grant boilerplate — government disclaimer text that should have been stripped at extraction. The cause was traced not by debugging current code, but by re-reading old extraction-step memory files. The bug had been a known unknown that drifted into a known: a route called `publish_as_is` skipped the `contamination_guard` step, and `consolidate_courses.py` bypassed `step_clean` entirely.
...
Without the audit log, I would have re-derived this from scratch. Cost would have been days, not hours.
...
I've run this experiment both ways. The weeks I scan first save me hours per week. The weeks I skip the scan and dive in re-derive the same lessons every two weeks. The interval is depressingly consistent — eighteen days for my most recent re-discovery.

Keystroke cost: the alias. Cognitive cost: sixty seconds. Everything else compounds.
...
<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  ...
  <p className="mt-3 text-white/70">One post every couple of weeks from a one-person studio that's been re-reading its own receipts long enough to recommend it. Memory architecture, agent failure modes, and the routing stack behind it all.</p>
