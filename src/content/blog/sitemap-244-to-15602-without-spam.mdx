...
244 URLs in the sitemap. 6,173 GSC impressions over 90 days. 8 clicks. 0.13% CTR. The kind of numbers that make you check whether the analytics tag is firing.
...
That last one was the opportunity. We had **15,000+ glossary terms already published** and rendering at canonical URLs. They had real definitions, reshaped from real instructor materials and SkillsCommons content. They were just not in the sitemap, so Google could find them only by crawling internal links — which it does, but slowly and incompletely.

The question was whether to add them all at once. The aggressive SEO answer is "yes, ship every URL." The cautious answer is "no, you'll get a thin-content penalty." The right answer turned out to be a third one.
...
A glossary entry on Qualora isn't a thin definition. It's a 100–300-word explanation reshaped from real human sources with cross-links to careers and courses. By the time we considered adding them to the sitemap, the underlying content was already published. The sitemap addition was an indexing question, not a content question.
...
Sitemaps support a `<priority>` value per URL. Google treats it as a hint about your own ranking of importance, not a guarantee of rank. The advice that doesn't get repeated enough: **Google trusts your sitemap less if you mark everything as priority 1.0**. If every URL is critical, none of them are.
...
The aggressive SEO playbook says ship every URL. The defensive playbook says be conservative with what you index. We ran a third option: **quality gates at ingestion, not at the sitemap boundary**.
...
Two follow-ups still on the list. First, structured-data validation across the 15K added URLs — we ran spot checks but not full coverage. Second, priority-budget tuning: if the 0.3 cap on glossary turns out to be too low after a few months of indexing data, we'll bump it to 0.4. The right number is whatever GSC actually rewards for our query mix, which we'll only know after the indexed volume stabilizes.
