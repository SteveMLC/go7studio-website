...
tags:
  ...
  - postmortem
status: "draft"
seoTitle: "Skeleton vs Content-First: Pipeline Routing Threshold | Go7Studio"
...
Two of those courses produced excellent learner-facing material. Two produced fluent, plausible, completely-disconnected-from-source garbage. The difference between them was a threshold we had not yet named, sitting silently in a `_manifest_docs/` word count we weren't reading.

This post is the story of how we discovered that threshold, the rich-source guard we now require, and the 16 pipeline commits behind one routing decision.

## §1 Two pipelines, four courses

We have two routes for moving a course from extracted source to publishable lesson:
...
What surprised me was that we shipped course 918 on the skeleton route initially, and it produced fluent, plausible, generic survey content. It read like a textbook. It bore essentially no relationship to the actual 94K-word IMSCC source.

That was when I learned what `skeleton_modernize` actually did.

## §2 What skeleton_modernize actually does

The misconception I'd been operating under: "skeleton_modernize uses the source material lightly — it preserves the lesson structure but smooths the prose and adds modern pedagogical scaffolding."
...
Notice what's not there: the source content. The actual material in `_manifest_docs/`. The original lessons from the source bundle.

This is *fine* when the source is thin — when there are 1,500 words of brittle, fragmentary source material with strong titles, and you want the LLM to fill in around the structural skeleton. The titles and learning objectives are real (preserved from source). The body is generated to match.

It is *catastrophic* when the source is rich — when there are 94K words of real source material that would have been the actual lesson content, and instead the LLM generates a generic substitute that ignores all of it.
...
That content is reasonable. It is not from any specific source — it is what a competent curriculum writer would produce for "Identifying Common Welding Defects" if asked. The pedagogy is standard. The factual claims are general and true. There are no fabricated specifics.
...
That's a textbook paragraph. It's accurate. It's also generic. Any high-school US History textbook contains this paragraph.
...
That's the actual lesson the original instructor wrote. It's specific. It's grounded in primary sources. It teaches the lesson the original course was built to teach.

The difference is not pedagogy. The difference is whether the LLM saw the source material.
...
When we route a course to `skeleton_modernize`, we are making a claim: this course has thin source, and the LLM should fill in around the structural skeleton. The claim is binding. If we route a course wrong, the LLM faithfully executes a wrong claim — fluently, plausibly, irrecoverably.

The 5K/20K threshold is not a tuning parameter. It is a guard. The rich-source guard in `skeleton_modernize.py` is what makes the threshold real. The env-flag override is what makes the guard auditable. The canary test (`canary_skeleton_rich_source_guard`) is what makes the override impossible to silently regress.
...
<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  ...
  <p className="mt-3 text-white/70">"When to let the LLM fill in vs when to extract real source" is the central question for any content-pipeline buyer. If your pipeline has multiple routes, talk to us.</p>
