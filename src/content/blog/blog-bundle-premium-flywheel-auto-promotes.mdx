...
A reader lands on a career story. The story has an AI bundle callout. The callout points at a bundle landing page. A nightly audit watches traffic and conversion. When a bundle clears its threshold, a flip script promotes it from free to premium tier. Future readers get a slightly better-monetized version of the same post. None of it requires a person in the loop on a normal day.
...
Four scripts, all of them in the Qualora repo. Below is what each one does and why we deliberately wrote them as small, dumb, single-purpose pieces.
...
The audit's only job is to write that JSON. It doesn't flip anything. It doesn't change tiers. It identifies candidates and stops.

The firewall is operational. If something's wrong with the audit logic — bad thresholds, broken metrics, drifted definitions — I want to read a JSON file and ignore it, not unwind a tier flip that should never have happened. Audits write. Triggers act. Different scripts.
...
No model call. No content generation. No surprises. The flip is data-layer only. UI surfaces re-render off the new tier flag at next request. The bundle itself doesn't change — its production passes were already done; the tier label is the only thing that moved.
...
If the Codex red-team produces findings, the bundle doesn't seed. It goes back to BUILD with the findings attached. We've caught our own numeric drift this way more than once.
...
A year from now this version will probably be unrecognizable. That's the point of writing it down.
