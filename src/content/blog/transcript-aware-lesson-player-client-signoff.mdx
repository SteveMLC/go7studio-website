...
tags:
  ...
  - whisper
status: "draft"
seoTitle: "The Transcript-Aware Lesson Player That Closed My Client | Go7Studio"
...
Three UX choices closed that demo. None of them are novel. All of them are missing from the default video player every other CE platform ships. This post is the engineering breakdown.
...
That's a 15-minute cost on a 60-second question. It's the friction that makes professional learners hate online CE.
...
**1. Auto-scroll.** As the video plays, the transcript pane scrolls so the active line stays centered. The learner doesn't have to chase the cursor; the cursor stays in their gaze and the text moves under it.
...
Three features. Conceptually small. UX-significant. And the same component layer hosts the compliance enforcement, which is the second-order win.
...
The reason it works as a closer is that it solves the actual job-to-be-done in two seconds, in front of them. Most edtech demos involve a lot of words about "engagement" and "outcomes" and "retention." Click-to-seek is just the platform doing the obvious right thing, visibly, immediately.

If you're building an edtech platform, this is what we recommend you ship first. Whisper transcripts cost roughly $0.36 per hour of audio at large-v3. The player layer is a day. The conversion-rate lift on demo close is the difference between "let me think about it" and "yes."
