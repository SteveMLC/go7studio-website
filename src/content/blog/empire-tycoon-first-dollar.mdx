...
That's the whole artifact. Five dollars from a stranger, recorded by hand because I hadn't yet wired up the BigQuery sync. The SKU was `premium_purchase` — the **legacy $4.99 "remove ads + 1 platinum point"** tier. The simplest, cheapest non-cosmetic IAP in the game.
...
The buyer didn't pick the most expensive thing. They picked the legacy ad-removal tier. That's information.

Empire Tycoon's actual IAP shelf, with real display prices straight from `BillingService`:
...
| `premium_purchase` (legacy) | **$4.99** | Remove ads + 1 platinum point |
| `pp_pack_large` | **$4.99** | 1500 platinum points |
| `premium_purchase_enhanced` (top tier) | **$6.99** | Remove ads + bigger platinum grant |

The most expensive thing in the store is the **enhanced premium tier at $6.99**, not the legacy $4.99. The cheapest is `pp_pack_small` at ninety-nine cents. The first buyer skipped past the dollar packs and the income boost and paid almost five times as much for the legacy ad-removal tier.

Why?

Because removing ads from an idle game is the IAP that earns its keep over the long tail of the session. Income boosts wear off. Platinum packs get spent. Ad removal compounds every minute you keep playing. The buyer wasn't optimizing for *cheapest*. They were optimizing for *most value per dollar over time*. That is a much more sophisticated read on the IAP carousel than I'd given a first-session player credit for.

By Day 90, of 37 paying buyers, the distribution looked like this:

| Tier | SKU | Buyers | Gross revenue |
| --- | --- | --- | --- |
| Top | `premium_purchase_enhanced` ($6.99) | 15 | $107.03 |
| Mid | `pp_pack_large` ($4.99) | 5 | $25.55 |
| Mid | `premium_purchase` ($4.99 legacy) | 4 | $24.17 |
| Mid | `starter_bundle` ($2.99) | 7 | $22.60 |
| Low | `income_boost_24h` ($1.99) | 4 | $8.36 |
| Low | `pp_pack_medium` ($2.49) | 1 | $2.94 |
| Low | `pp_pack_small` ($0.99) | 1 | $1.99 |
| **Total** | — | **37** | **$192.63** |

Premium tiers (enhanced + legacy) carried 19 of the 37 buyers and $131.20 of the $192.63 gross. The enhanced version eventually out-converted the legacy version fifteen to four — because per platinum point delivered, the enhanced version was actually a better deal. The buyers were doing math.

The first buyer wasn't an outlier. They were the first proof of a pattern that took ninety days to fully read.

> [!NOTE]
> Gross figures here come from BigQuery's `event_value_in_usd`, which includes regional pricing variation, so the totals run a few percent above clean per-unit math. The unit prices in the SKU table are the US display prices.

## The one thing I changed the next day

The IAP prompt had been firing on the prestige screen — once a player completed their first reset. The first buyer never prestiged. They saw an entry-tier offer because I'd also planted it on the post-build summary screen, almost as an afterthought. That afterthought was where the conversion actually happened.

So on Day 6 I moved the post-build placement earlier in the loop, made it the primary, and pushed the prestige-screen prompt to secondary. Buyer activation jumped on the change. By Day 90 the same shelf had carried thirty-seven of them.
...
Empire Tycoon is still my side project. The boys still ship me notes. There are now thirty-seven strangers in the data who paid me a total of $192.63 to keep building it. Some weeks that feels like nothing. Most weeks, ten months in, it feels like a lot.

<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  <h3 className="text-xl font-semibold text-white">Play the game thirty-seven strangers paid for</h3>
  <p className="mt-3 text-white/70">Empire Tycoon is the idle-game side of Go7Studio &mdash; eleven businesses, prestige loops, real estate, hustle taps. The version those buyers paid for. Free, ad-supported, runs on Android.</p>
