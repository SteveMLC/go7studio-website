...
Sixty Python scripts. Seven hundred public-domain courses moved from raw bundles to publishable lessons. One operator.

That last number is the one doing all the work. It only holds up because the pipeline isn't a script — it's a state machine with refusal rights, scripts hanging off it, and a contract for what each script promises and what it's allowed to refuse.

This is the tour. The 11 phases. The 12-status state machine that arrived around script #40 and quietly rewrote everything before and after it. The 16 extractors hiding behind one dispatcher. The halt conditions that turn silent failure into loud refusal. The canary harness that keeps every postmortem from coming back.

If you're designing a pipeline that has to survive real-world inputs — public-domain coursework, scraped corpora, customer bundles, anything that arrives in shapes you didn't choose — this is the architecture I'd build today.

<HeroCallout eyebrow="Pipeline architecture" title="The factory works because every phase can refuse." body="Sixty scripts only stay operable when state lives in a table, phases own narrow contracts, and every postmortem plants a canary that keeps the failure from coming back." />

<KeyTakeaways title="The contracts that keep one operator sane" items='[{"title":"State is a row","body":"The pipeline answers status questions from pipeline_items instead of scattered logs and memory."},{"title":"Refusal is a feature","body":"Every phase decides whether it can safely continue before it produces output."},{"title":"Dispatch is safety logic","body":"Format routing runs most-specific first so a familiar extension cannot hide the wrong extractor."},{"title":"Every bug plants a canary","body":"Postmortems only close after the original failure has a small reproducible test."}]' />
...
Refusal is a feature, not a failure. Half the value of this pipeline is in *what doesn't get published* — courses that look fine to a human reading metadata but that the validators caught with zero surviving words, or thin sources, or contamination, or extractor mismatch. The whole shape of the system is built around making refusal easy and silent-acceptance hard.
...
When this table got created, I had 772 in-flight courses scattered across log files and ad-hoc tracking spreadsheets. The backfill was its own multi-day project: parse every log file, parse every spreadsheet, reconcile against actual database state, write 772 rows with the best-known status. A consolidation guard kept two backfill rows from overwriting each other when the same course appeared in two log streams.
...
The biggest historical bug in this table is `intent_extract surviving_words=0 → continue` (Bug 8). It was supposed to halt and didn't. Modernize generated coherent garbage from titles. Twelve courses got bypass-routed past the consolidator and made it all the way to publish before the halt-condition was wired correctly.

The general rule: **every step in your pipeline should be able to answer "should I refuse?" before it answers "what is my output?"** If a step cannot refuse, it cannot be safe.

## §6 The canary harness

Every postmortem plants a test.

That's the rule. There are 39 canaries on this pipeline now — one per fixed bug, plus a handful of broader integration canaries. They run before any pipeline-touching merge. They finish in 8 seconds.
...
Every audit step was running 20× slower than baseline. Modernize was taking longer than usual. Validate was taking twice as long. Canaries all green, wall-clock numbers terrible.
...
You don't need 60 scripts to start. You need a state machine, a halt-on-zero default, and one canary. The 60 scripts will arrive on their own as the catalog grows. The architecture is what decides whether they're a working pipeline or a stack of duct tape.

<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  <h3 className="text-xl font-semibold text-white">Pipeline-engineering as a service</h3>
  <p className="mt-3 text-white/70">If your content pipeline is leaking money on bad routes, fabricated output, or scripts nobody remembers to run — that's the work I do at Go7Studio. Small studio, real receipts, every fix gets a canary.</p>
