...
"Which Disney princess matches your work style?" is entertainment. It produces engagement, not direction. Most career quizzes that ship as engagement features fall in the same bucket — optimized for completion rate and shares, not for telling somebody something true about their career.
...
We wanted assessment. The person taking the quiz on Qualora is usually trying to figure out what to do with the next phase of their working life. A vibes-based answer is worse than no quiz — it produces false confidence and routes them toward courses that don't fit.
...
This is a lesson I keep having to relearn. The temptation is always to build the best version of the feature first. The right move is to ship the lowest-quality version that proves the funnel, then invest in the feature itself once you've measured the funnel converting at any quality bar. Phase 1 had Phase 1 traffic data before we wrote a Phase 2 question.
...
That's the whole matcher at the math layer. Eleven lines. The thing that makes it work isn't the code — it's that the 34 career-profile vectors are seeded from O\*NET-SOC, the dataset states use to make labor-policy decisions. The signal lives in the input data, not the algorithm.
...
Three matches at 98–99% isn't noise. It's the system reproducing established, externally-verified mappings from a public-domain occupation database.

If the matcher had returned "ICR → Welder, 87%," that would be a bug signal — Welder is more Realistic-dominant. The canonical matches coming back at 98%+ on canonical inputs tells us the vector seeding is faithful to the source data and the cosine math is doing what it should.
...
1. **Use the existing standard.** O\*NET is free, public domain, validated, and the dataset state workforce systems already use. If you're inventing your own framework, you're either inventing a worse O\*NET or marketing a brand around the same six dimensions with different names.
...
If you've taken our quiz, the math above is what produced your results. If you haven't, the link's a few paragraphs up.
