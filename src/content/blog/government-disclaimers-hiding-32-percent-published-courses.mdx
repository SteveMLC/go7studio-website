...
<HeroCallout eyebrow="Corpus cleanup" title="Boilerplate contamination is a boundary failure, not a copyediting task." body="The fix was to move the guard to every ingest path, document what the stripper deliberately keeps, and clean the published rows without regenerating learner-facing content." />

<DecisionMatrix title="Where the guard belongs" columns='["Boundary","Guard that belongs there","Bypass it catches"]' rows='[{"label":"publish_as_is","values":["Run contamination check even when modernize is skipped.","Clean-looking source that still carries grant or license boilerplate."]},{"label":"consolidator","values":["Clean each merged bundle before modernize and seed.","Multi-bundle courses that bypass the normal step_clean route."]},{"label":"bulk remediation","values":["Snapshot affected rows, strip known patterns, write a touched timestamp.","Large-corpus cleanup that would otherwise require nondeterministic re-generation."]},{"label":"pattern file","values":["Keep an explicit list of non-matches next to the regex rules.","Accidentally deleting real attribution while removing funding metadata."]}]' />
