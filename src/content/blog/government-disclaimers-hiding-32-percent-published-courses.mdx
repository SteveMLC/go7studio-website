...
tags:
  ...
  - postmortem
status: "draft"
seoTitle: "Government Disclaimers in 32% of Published Courses | Go7Studio"
...
I was auditing course 936 for an unrelated formatting issue when I noticed a paragraph that did not belong.
...
That is a TAACCCT grant disclaimer. It belongs on the original source materials, not in our learner-facing course body. The strange part: the disclaimer was repeating, almost verbatim, three times across four lessons. Course 936 was at 100% boilerplate density at the section level — almost every lesson opened or closed with grant-acknowledgement language.
...
This post is the postmortem: the two bugs that let it through, the 18-pattern regex we built to strip it, and the bulk SQL operation that cleaned the corpus without re-running the pipeline.
...
That disclaimer is fine — required, even — on the original PDF, the SCORM package, the syllabus PDF. It is not fine when it gets ingested as lesson body text and presented to a learner as if it were instructional content.
...
Three problems with that block. The grant-acknowledgement paragraph is metadata, not instruction. The grant agreement number is internal grant-administration data the learner does not need. The CC license text is correct attribution, but it belongs in a footer, not at the top of every lesson.

Multiply that across 212 courses and you have a corpus that reads like it was scraped, not curated.
...
The first bug was in our pipeline router. Qualora's pipeline has a step called `step_clean` that runs `strip_metadata.py` (now also `strip_grant_boilerplate.py`). It is supposed to run on every course before modernize.

But there is a `publish_as_is` route. The intent was sensible: if a course is already in clean, modernize-ready shape (which sometimes happens when a course was authored natively for our platform), the operator can skip cleaning and go straight to enrich-and-seed.
...
The router treated cleaning as optional when it was load-bearing. `publish_as_is` was correct in spirit — these courses do not need *modernize* — but it should never have skipped *contamination check*.
...
Many of our published courses were built by `consolidate_courses.py`, which merges multiple source bundles into one course. (Example: a TAACCCT grantee shipped twelve separate SCORM packages for a single curriculum; we consolidate them into one Qualora course.)

`consolidate_courses.py` calls `modernize.py` directly. It does not call `step_clean`. It does not call `strip_metadata.py`. It bypasses the entire data-cleaning phase.
...
This was the architecture-level cause of most of the 212 contaminated courses. They were not random — they were almost all consolidated multi-bundle courses, and consolidator's amnesia meant the cleaning step never ran.
...
The non-matches are the more interesting design call. Plenty of grant-funded courses are also legitimately attributed to the original instructor or the original college. We deliberately did not strip those. `instructor: Jane Doe` stays because it is real attribution. `merlot.org/...` stays because MERLOT is a real source repository for some of our materials and we want the citation to survive.

The line is: **strip metadata that exists only because of the funding mechanism. Keep metadata that exists because of the human contribution.**

This is the same line you draw if you are filtering PII while preserving authorship, or filtering license text while preserving citation. The patterns differ. The principle is the same: be specific about what you are removing and why, and keep an explicit "what we deliberately keep" list in the same file as your strip patterns.
...
<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  ...
  <p className="mt-3 text-white/70">If your LLM pipeline runs on real-world content — public-domain coursework, scraped corpora, regulatory documents — contamination guards at every ingest boundary are the work. That's what we do at Go7Studio.</p>
  <Link href="/contact" className="btn-primary mt-6 inline-flex">Talk to Go7Studio</Link>
</div>

The 212-course corpus is now stripped. The bulk operation takes about three minutes to run. The two route bugs are closed. Both got canary tests planted in `run_canaries.py` so they cannot recur silently.

That last part — the canary test — is the rule we now run every postmortem against. Every postmortem must plant a test, or it will recur. That's the topic of [the 19-bug failure-pattern atlas](/blog/nineteen-bugs-failure-pattern-atlas).
