...
<HeroCallout eyebrow="Extractor economics" title="Routing is the safety system before extraction starts." body="The cheapest extractor is worthless if the dispatcher sends a source bundle down the wrong path. The matrix only works because detection order, fallback thresholds, and known gaps are explicit." />

<DecisionMatrix title="Routing decisions that carry the cost" columns='["Decision","Why it matters","Failure it prevents"]' rows='[{"label":"Storyline before IMSCC","values":["Both can arrive as zip-shaped bundles, but Storyline needs a more specific detector.","A Storyline course silently returning empty content through the IMSCC path."]},{"label":"Text-layer PDF before OCR","values":["Most PDFs do not need vision OCR, so the cheap parser gets first pass.","Paying 4x per word when a normal text layer already exists."]},{"label":"OCR fallback below threshold","values":["Image-only PDFs are routed to vision only when word count proves the text layer is thin.","Publishing zero-word courses from scanned source material."]},{"label":"Storyline 2+ as a named gap","values":["Correct detection plus incomplete extraction is visible as triage, not a mystery failure.","Confusing unsupported runtime data with a working extractor."]}]' />
