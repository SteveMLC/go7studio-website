...
tags:
  ...
  - format-zoo
status: "draft"
seoTitle: "16 Extractors, 8 Course Formats: Cost Matrix | Go7Studio"
...
When you ingest real public-domain courses — not curated benchmark datasets — they don't arrive in one format. They arrive in eight. We've ended up with sixteen extractors across those eight formats, plus an OCR fallback for the cases where there isn't actually any text in the file.

This post is the cost matrix. What each format looks like in the wild, which extractor handles it, what it costs to run per course, and the gotchas we've discovered the hard way. The numbers are real. The Storyline 2+ gap is real and currently unfixed.

## §1 The format zoo

Real public-domain content does not arrive in one format. It arrives in whatever the original grantee, instructor, or community college had on their authoring tool when they exported. Eight years of TAACCCT-funded courseware means eight years of every authoring tool in education exporting in their preferred format.
...
Each extractor has its own quirks — its own format-specific tricks for finding actual content in whatever the source authoring tool produced.
...
The cost-per-word for OCR is higher than for text-layer (about 4× per word), but the alternative is zero words. The economics are: **OCR is the unlock, not the optimization.** Image-only PDFs were producing nothing before this fallback existed.

## §4 PDF-OCR economics

The OCR fallback is the single most-impactful extractor in the matrix. It is also the cheapest LLM-vision call we make.
...
Second priority: a `.dat` parser improvement for Blackboard. The current implementation handles the common case. Some Blackboard exports use newer `.dat` schemas with embedded XML islands; we currently pass those to the generic walker and lose structure. About 12 courses in triage are blocked here.

Third priority: a real EPUB extractor for the long-form coursework that comes as EPUBs. We have a thin wrapper around the spine but lose embedded media. Probably 6-8 courses blocked.

The pattern across all three: each new extractor is a constant cost ($0.01-$0.10/course) for an entire blocked cluster of courses. The unit economics work out in single-digit dollars to unlock dozens of courses. Compared to the cost of *not* extracting them (zero published content from those bundles), the math is straightforward.
...
<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  ...
  <p className="mt-3 text-white/70">Storyline 2+ unlock is in our roadmap. If your corpus is blocked on a format we haven't covered yet — Captivate, Lectora, custom HTML5 wrappers, anything — talk to us.</p>
