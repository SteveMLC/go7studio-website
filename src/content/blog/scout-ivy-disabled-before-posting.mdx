...
tags:
  ...
  - ai-orchestration
status: "draft"
seoTitle: "Why I Disabled Two Finished SEO Agents | Go7Studio"
...
Scout and Ivy were architecturally finished. Tests passed. Cron lines were ready. Both were going to push content under brand accounts on a recurring schedule. Neither posted anything. I disabled both on day two of their build.

This post is why that happened, the four-question pre-flight test that would have caught it earlier, and the negative-space lesson that came out of building two finished agents that weren't ready to be alive.

## Two finished agents on the shelf

Scout and Ivy were the first two-thirds of a three-stage SEO pipeline. Scout was supposed to scan target communities (Qualora-relevant subreddits, Quora topics) on a six-hour cron, score posts, and fill a queue. Ivy was supposed to pick from the queue, draft platform-native replies, and hold them for human approval.
...
An agent without an output channel generates costs without value. Eight runs per day at LLM-call rates is non-trivial spend with no offsetting output. Worse, eight runs per day of draft generation against an empty downstream pipeline means a queue that grows indefinitely, dry-run logs that fill disk, and an agent that *looks* productive while accomplishing nothing.

The decision was the right one within hours. Disable both. Document the prerequisites. Wait until the surfaces are real. Re-enable when the four-question pre-flight passes.
...
> [!IMPORTANT]
> Build the surface area, then audit if it deserves to be live. Most failed agents weren't bad code — they were correct code without a real-world hook. Scout and Ivy will come back online after Petra's warmup phase completes ([the awkward repair on the publish tier](/blog/petra-reddit-signup)).

The deeper lesson is about the order I built things in. I built the agents before I built the accounts. That feels backwards now, but in the moment it felt natural — agents are the interesting work, accounts are paperwork. Building the agents first meant the agents were ready to fire before there was anywhere for them to fire at, which is exactly the failure mode the pre-flight test catches.
...
The publish tier (Petra) is now mid-warmup on a real Reddit account. Once Petra's gates clear, Scout and Ivy come back online — not because the agents got better, but because the surfaces became real.
...
Cron-running agents with no fire-able output are worse than no agents — they generate cost, fill logs, and create a false sense of activity that masks real bottlenecks. Better to disable cleanly with a written re-enable plan than to leave them running because shutting them down feels like admitting failure.
...
<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  ...
  <p className="mt-3 text-white/70">The lab covers agent design, fire-able signals, and the routing stack behind a one-person studio. New post every couple of weeks.</p>
