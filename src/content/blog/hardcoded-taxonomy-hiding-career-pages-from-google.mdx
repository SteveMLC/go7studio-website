...
GSC said 8 indexed careers. The admin dashboard said 31 published. Twenty-three careers were live in production and Google didn't know any of them existed.
...
When two numbers that should agree don't, the question is which one is lying. The database was right. The 8 came from somewhere else. The hunt for "where" took longer than the fix.
...
Four copies of the same array. None of them imported from a shared module. If you added a career to one, it had to be added to the other three. Nobody had touched any of them in months because the production seeder publishes career pages directly off database state — and that flow had been working fine.

So new careers shipped. They rendered fine for any user who landed on `/careers/<slug>`. They just never made it into the homepage's "Featured Careers" grid, the sitemap, the static-route metadata, or the glossary funnel. From Google's perspective, the database had 31; the only ones discoverable from internal links and the sitemap were the original 8.
...
That's the worst kind of bug: nothing 500s, nothing logs, the page is up. It just isn't discoverable.
...
If you're shipping anything where new entities — products, locations, courses, careers, ingredients, models — land in a database and your homepage / sitemap / SEO layer has a list, audit it today. Search your codebase for any array literal with more than three slugs in it. If you find one, that's almost certainly a future stale-taxonomy bug. Move it to a query before it costs you index coverage.
