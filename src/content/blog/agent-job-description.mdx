...
tags:
  ...
  - studio-meta
status: "published"
seoTitle: "I Wrote My AI Agent a Job Description | Go7Studio"
...
sequenceDiagram
    ...
    participant W as Walt
    participant C as Chip
    Note over S,C: Before JD — Walt does the work
    S->>W: "Need that pipeline fix today."
    W->>W: opens repo, writes diff, commits
    W-->>S: "Done."
    Note over S,C: Outcome: Chip's QC gates skipped, drift accumulates

    Note over S,C: After JD — Walt routes the work
    S->>W: "Need that pipeline fix today."
    W->>C: writes inbox/chip/work-order-fix-X.md
    C->>W: writes inbox/walt/result-fix-X.md (PR + canary)
    W-->>S: "Routed to Chip; PR open, canary green."
    Note over S,C: Outcome: every agent's QC gate fires
```
