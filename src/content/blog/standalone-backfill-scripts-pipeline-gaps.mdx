...
tags:
  ...
  - operations
status: "draft"
seoTitle: "Standalone Backfill Scripts Are Pipeline Gaps | Go7Studio"
...
When I stopped manually triggering them — because I had moved on to other work, because they had become invisible, because they did exactly the right thing every time and never needed attention — the pipeline silently regressed. Two courses shipped with 206 and 271 images respectively, most of them logos and decorative junk.
...
Both scripts were written for a one-off image-quality cleanup back in January. They worked. They produced clean output. I'd run them manually after each ingestion wave, see the junk-image counts drop, and move on.

Then I stopped running them. Other work. Other waves. The scripts didn't change. The pipeline didn't change. And gradually it became invisible that the pipeline didn't include image classification at all — that all the image-quality work was being done by my human muscle memory.

## §2 Discovery — Bug 19

HIT 101 published. 206 images on the course. Most of them logos. Some of them decorative shapes lifted from the original PowerPoint backgrounds. A handful of actual diagrams, buried in the noise.
...
I went to look at why the image filter wasn't running. There was no image filter. There never had been an image filter *in the pipeline*. The image filter was sitting in `/scripts`, waiting for me to remember to run it.

That moment — looking at the pipeline DAG and seeing no node for the work that was clearly happening every time we shipped a clean course — is the discovery. The script wasn't a backup. The script was the *pipeline gap*. The whole time I'd been "running it manually," I was patching a hole I hadn't admitted existed.
...
The cost of running this on every course:
...
Two-tenths of a cent per typical course. About $2/year to run this for the whole catalog at current volume.

The cost of *not* running it: 136 junk images shipped to learners on HIT 101, 186 on HIT 105. The user-facing cost is real (cluttered lesson pages, slower load, weird logos repeated everywhere). The CDN cost is real (storing and serving images that nobody is supposed to see). The cost of fixing it after publish is huge (un-publishing, re-running, re-publishing, link rot).

Two-tenths of a cent vs all of that. It is one of the cheapest pipeline steps we run, and it was the most expensive one we weren't running.
...
When the operator stops being the pipeline node, the pipeline regresses. That's not a flaw in the operator. That's a flaw in the architecture that depended on the operator's invisible labor.
...
<div className="my-12 rounded-2xl border border-brand-teal/30 bg-brand-teal/5 p-8">
  ...
  <p className="mt-3 text-white/70">If your `/scripts` directory has tools doing pipeline-shaped work, they're pipeline gaps. We wire them in, plant canaries, and make the work invisible to the operator the right way — by automating it.</p>
